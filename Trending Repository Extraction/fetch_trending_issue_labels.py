...
csv_file = 'trending_metadata.csv'
updated_csv_file = 'trending_repos_with_issues.csv'
...
with open(csv_file, mode='r', newline='', encoding='utf-8') as input_file:
    ...
    with open(updated_csv_file, mode='w', newline='', encoding='utf-8') as output_file:
        ...
        for row in reader:
            full_name = row['author'] + '/' + row['name']
            repo_url = f'https://github.com/{full_name}/labels?sort=count-desc'
            print(f"Fetching labels for {full_name}:")
            try:
                ...
                print(f"Error fetching labels for {full_name}: {e}")
